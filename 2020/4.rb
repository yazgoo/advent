...
valid_count = 0
categories = []
passports.each_line do |line|
  line = line.chomp
  if line == ""
    cat_count = must_have.map { |cat| (categories.include? cat) ? 1 : 0 }.reduce(:+)
    valid_count += 1 if must_have.size == cat_count
    categories = []
  else
    categories += line.split(" ").map { |x| x.split(":")[0] }
  end
end
p valid_count
...
passports.each_line do |line|
  line = line.chomp
  if line == ""
    present = must_have.size == must_have.map { |cat| (fields.keys.include? cat) ? 1 : 0 }.reduce(:+)
    valid = fields.map do |key, value|
      case key.to_sym
      when :byr
        v = value.to_i
        value.size == 4 && 1920 <= v && v <= 2002
      when :iyr
        v = value.to_i
        value.size == 4 && 2010 <= v && v <= 2020
      when :eyr
        v = value.to_i
        value.size == 4 && 2020 <= v && v <= 2030
      when :hgt
    ...
    valid = valid.reduce(:&)
    valid_count += 1 if present and valid
    fields = {}
  else
    res = Hash[line.split(" ").map { |x| x.split(":") }]
    fields.merge!(res)
  end
end
p valid_count
