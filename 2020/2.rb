...
total = 0
policies.each { |policy_pass|
  policy, string = policy_pass
  policy = policy.split(" ")
  letter = policy[1]
  numbers = policy[0].split("-").map { |n| n.to_i }
  count = string.scan(letter).length
  total += 1 if count >= numbers[0] and count <= numbers[1]
}
p total
...
policies.each { |policy_pass|
  ...
  policy = policy.split(" ")
  letter = policy[1]
  numbers = policy[0].split("-").map { |n| n.to_i - 1 }
  total += 1 if (string[numbers[0]] == letter) ^ (string[numbers[1]] == letter)
}
p total
